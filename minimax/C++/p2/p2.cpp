...
minimax_abeta(Reversi init, int player, int depth, int alfa, int beta)
{
    ...
    for (int i = 0; i < N; i++)
        ...
        {
            Reversi tmp = init;
            bool applied = tmp.apply_move(Move(player, i, j));
            if (applied)
            {
                has_move = true;
                std::pair<int, Move> p = minimax_abeta(tmp, -player, depth - 1, -beta, -alfa);
                if (-p.first >= ret.first)
                {
                    ret.first = -p.first;
                    ret.second = Move(player, i, j);
                }
                if (-p.first > alfa)
                    alfa = -p.first;
                if (alfa >= beta)
    ...
    {
        Reversi tmp = init;
        tmp.apply_move(Move(player));
        std::pair<int, Move> p = minimax(tmp, -player, depth - 1);
        if (-p.first >= ret.first)
        {
            ret.first = -p.first;
            ret.second = Move(player);
        }
    }
