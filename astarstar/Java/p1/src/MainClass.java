...
class StateComparator implements Comparator<State> {
	...
	private int computeF(State state) {
		...
		case AStar:
			...
			result = state.getDistance()
					+ state.approximateDistance(solutionState);
		}
		return result;
	}
...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		Comparator<State> stateComparator = new StateComparator(
				StateComparator.Algorithm.AStar, solutionState);
		...
		/* TODO: A* */
		ArrayList<State> vecini;
		State n = initialState;
		int g = 0, cost = 0;
		
		while (true) {

			if (open.size() == 0 && n != solutionState) {
				System.out.println("failure");
				break;
			}

			n = open.poll();
			if (n.x == solutionState.x && n.y == solutionState.y) {
				n.printPath();
				break;
			} else {
				...
				vecini = n.expand();
				for (State s : vecini) {
					cost = s.getDistance() + 1;
					if (!closed.contains(s) && !open.contains(s)) {
						s.setParent(n);
						s.setDistance(cost);
						open.add(s);
					} else {
						if (cost < s.getDistance()) {
							s.setParent(n);
							s.setDistance(cost);
							closed.remove(s);
							open.add(s);
